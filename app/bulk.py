...
from app.main import get_db, verify_token, Depends
...
async def bulk_upload(file: UploadFile = File(...), user: dict = Depends(verify_token)):
    ...
    # 4. Read back from Minio and process
    try:
        mc = get_minio()
    ...
    # 6. Validate rows (skip header = row 0)
    ...
    # Merge validation errors with insert results
    for err in errors:
        results.append({
            "row": err["row"],
            "sku": err["sku"],
...
def _map_columns(header: list[str]) -> dict | None:
    """Map header names to expected column indices."""
    mapping = {}
